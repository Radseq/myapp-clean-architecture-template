...
/// <summary>
/// Jednorazowe przetworzenie partii outboxów. Scoped (bo używa DbContext).
/// </summary>
    ...
    ILogger<OutboxProcessor> logger)
{
    ...
    public async Task<int> RunOnceAsync(CancellationToken ct)
    {
        ...
        // 1) Acquire batch (krótka transakcja, retry-safe)
        var batch = await AcquireBatchAsync(now, lockId, leaseUntil, ct);
        if (batch.Count == 0)
            return 0;

        // 2) Dispatch poza transakcją (żeby nie trzymać locków na DB)
        var processed = 0;
    ...
    private async Task<List<OutboxMessage>> AcquireBatchAsync(
        DateTime nowUtc,
        Guid lockId,
        ...
        CancellationToken ct)
    {
        ...
        return await executionStrategy.ExecuteAsync(async () =>
        {
            ...
            var q = db.OutboxMessages
                .Where(m => (m.Status == (byte)OutboxStatus.Pending || m.Status == (byte)OutboxStatus.Failed) &&
            ...
            {
                m.Status = (byte)OutboxStatus.Processing;
                m.LockId = lockId;
    ...
    private async Task ApplyResultAsync(Guid id, Guid lockId, OutboxDispatchResult result, CancellationToken ct)
    {
        ...
        await executionStrategy.ExecuteAsync(async () =>
        {
            ...
            if (entity is null)
                return; // ktoś inny przejął / lease wygasł / już przetworzony
    ...
    {
        // prosty exponential backoff z capem
        // 1->MinBackoff, 2->2*MinBackoff, 3->4*MinBackoff, ...
        var mult = 1 << Math.Min(attempt - 1, 10); // cap shift żeby nie overflow
