...
internal static class LoggingRedaction
{
    ...
    public static string SanitizeQueryString(IQueryCollection query, HashSet<string> denyKeys, int maxLen)
    {
        ...
        foreach (var (key, values) in query)
        {
            ...
            var raw = values.Count switch
            {
                0 => string.Empty,
                1 => values[0],
                _ => string.Join(",", values.ToArray()) // FIX: jednoznaczny overload
            };
