...
using MyApp.BuildingBlocks.Presentation.Http;
using MyApp.Modules.Orders.Contracts;
using MyApp.BuildingBlocks.Presentation.Versioning;


namespace MyApp.Modules.Orders.Presentation.Controllers.V1;
...
public sealed class OrdersController(IMediator mediator, IMapper mapper) : ControllerBase
{
        ...
        CancellationToken ct)
    {
        return await this.SendCreatedAtAction(
            mediator,
            request: mapper.Map<Application.Features.CreateOrderAndDispatchTransport.Command>(dto),
        ...
        CancellationToken ct)
    {
        return await this.SendNoContent(
            mediator,
            request: mapper.Map<Application.Features.CreateOrder.Command>(dto),
            ct);
    }
    ...
    public async Task<IActionResult> GetById([FromRoute] int id, CancellationToken ct)
    {
        return await this.SendOk(
            mediator,
            new Application.Features.GetOrderById.Query(id),
