...
namespace MyApp.Presentation.Controllers.V2;
...
public sealed class OrdersController(IMediator mediator, IMapper mapper) : ControllerBase
{
		...
		CancellationToken ct)
	{
		return await this.SendCreatedAtAction(
			mediator,
			request: mapper.Map<Modules.Orders.Application.Features.CreateOrderAndDispatchTransport.Command>(dto),
	...
	public async Task<IActionResult> GetById([FromRoute] int id, CancellationToken ct)
	{
		return await this.SendOk(
			mediator,
			new Modules.Orders.Application.Features.GetOrderById.Query(id),
