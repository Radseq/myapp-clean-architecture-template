...
public sealed class OutboxDispatcher<TDbContext, TMsg, TModule>(
	TDbContext db,
	IEnumerable<IOutboxMessageHandler<TModule>> handlers,
	IOptionsMonitor<OutboxOptions> options,
	IOutboxModuleOptions<TModule> moduleOpt,
	TimeProvider timeProvider,
	ILogger<OutboxDispatcher<TDbContext, TMsg, TModule>> logger)
	: IOutboxDispatcher<TModule>
	...
	where TMsg : class, IOutboxMessageEntity
{
	...
	public async Task<bool> TryDispatchOnceAsync(Guid outboxId, CancellationToken ct)
	{
		...
		var envelope = new OutboxEnvelope(
			msg.Id, msg.Type, msg.PayloadJson, msg.IdempotencyKey, msg.AttemptCount);
	...
	private async Task<TMsg?> AcquireSingleAsync(
		Guid id,
		DateTime nowUtc,
		Guid lockId,
		...
		CancellationToken ct)
	{
		...
		return await strategy.ExecuteAsync(async () =>
		{
			...
			var msg = await set.SingleOrDefaultAsync(m =>
				...
				(EF.Property<DateTime?>(m, nameof(IOutboxMessageEntity.LockedUntilUtc)) == null ||
				 EF.Property<DateTime?>(m, nameof(IOutboxMessageEntity.LockedUntilUtc)) < nowUtc), ct);

			if (msg is null)
				return null;

			msg.Status = OutboxStatusCodes.Processing;
			msg.LockId = lockId;
